/********************************** (C) COPYRIGHT *******************************
 * File Name          : RF_extend.c
 * Author             : WCH
 * Version            : V1.0
 * Date               : 2025/06/24
 * Description        : 2.4G库扩展模式收发测试例程
 *
 *                      1. 功率设置
...
void rf_rx_process_data( void )
{
    gRxCount ++;
    {
        ...
        #if(DPL_EN)
            Rssi_sum += (int8_t)pData[TEST_DATA_LEN+RSSI_DPL_OFFSET];
        #else
...
void RFRole_Init(void)
{
    ...
    PKT_DET_CFG4(0x78);
#endif
    if(MODE_2G4 == PHY_2G4_1M)
    {
        #if( AA_LEN== ADDR_3BYTE )
            PKT_DET_CFG3(124); //3字节地址
        #else
            PKT_DET_CFG3(156); //4&5字节地址
        #endif
    }
    else {
        #if( AA_LEN== ADDR_3BYTE )
            PKT_DET_CFG3(128); //3字节地址
        #else
            PKT_DET_CFG3(160); //4&5字节地址
        #endif
    }
    ...
    {
       Properties.cfgVal =  TEST_PHY_MODE |BB_WHITENING_OFF; 
